...
function App() {
  ...
  const [routeSelected, setRouteSelected] = useState<Route | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    ...
    socketIORef.current = ioConnect('http://localhost:3333');
  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (routeSelected && mapRef.current && socketIORef.current) {
